...
def get_available_timestamps_for_activity(your_court_id):
    conn = psycopg2.connect(
        host='your_postgres_host',
        user='your_postgres_user',
        password='your_postgres_password',
    ...
    query = f"SELECT datetime FROM bookings WHERE court_id = {your_court_id} AND availability = TRUE"

    cursor.execute(query)
    available_timestamps = cursor.fetchall()
